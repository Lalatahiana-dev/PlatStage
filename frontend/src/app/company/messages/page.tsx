...
export default function CompanyMessagesPage() {
  ...
  useEffect(() => {
    ...
    const loadConversations = async () => {
      try {
        const res = await api.get(`/conversations/company/${companyId}`);
  ...
  const filteredConversations = conversations.filter((conv) => {
    ...
    const fullName =
      `${conv.student.user.prenom} ${conv.student.user.nom}`.toLowerCase();
    return (
      fullName.includes(q) ||
    ...
    return (
      <div>
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-500 mt-0.5">Échangez avec les étudiants</p>
        </div>
        <div className="min-h-[60vh] flex items-center justify-center">
          <div className="text-center max-w-md px-4">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Créez votre profil entreprise pour accéder aux messages.
            </p>
            <a
          ...
          <div className="flex-1 overflow-y-auto" role="listbox" aria-label="Conversations">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 px-6">
                ...
                <p className="text-xs text-gray-400 mt-1 text-center">
                  {searchQuery
                    ...
                    : "Les messages des candidats apparaîtront ici"}
                </p>
              </div>
            ) : (
              filteredConversations.map((conv) => {
                ...
                const fullName = `${conv.student.user.prenom} ${conv.student.user.nom}`;
                return (
                  <motion.button
                    ...
                    <div className="relative flex-shrink-0">
                      ...
                      >
                        <span className="text-white font-semibold text-sm">
                          {getInitials(
                            conv.student.user.prenom,
                            conv.student.user.nom,
                          )}
                        </span>
                      </div>
              ...
              <div className="flex-shrink-0 px-4 sm:px-6 py-3.5 flex items-center gap-3.5 border-b border-gray-100 bg-white/80 backdrop-blur-sm">
                ...
                <div className="relative flex-shrink-0">
                  ...
                  >
                    <span className="text-white font-semibold text-sm">
                      {getInitials(
                        selected.student.user.prenom,
                        selected.student.user.nom,
                      )}
                    </span>
                  </div>
