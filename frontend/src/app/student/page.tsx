    ...
    <div>
      ...
      <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-6 mb-6 flex justify-between items-center">
        <div>
          ...
          <div className="flex gap-3">
            <Link href="/student/offers" className="px-4 py-2 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700 transition">
              Découvrir les offres
            </Link>
            <Link href="/student/profile" className="px-4 py-2 border border-indigo-300 text-indigo-600 text-sm rounded-lg hover:bg-indigo-50 transition">
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { num: '1', label: 'Créez votre profil', desc: 'Inscrivez-vous et complétez votre profil en quelques minutes.', icon: 'ti-user', color: 'indigo' },
            { num: '2', label: 'Trouvez des offres', desc: 'Consultez les offres qui correspondent à votre profil.', icon: 'ti-search', color: 'green' },
            { num: '3', label: 'Postulez facilement', desc: "Envoyez votre candidature en ligne en quelques clics.", icon: 'ti-send', color: 'yellow' },
            { num: '4', label: 'Suivez votre progression', desc: "Suivez l'état de vos candidatures et entretiens.", icon: 'ti-chart-bar', color: 'purple' },
          ].map((step) => (
            <div key={step.num} className="bg-white border border-gray-100 rounded-xl p-4 text-center">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-2 bg-${step.color}-50 text-${step.color}-600`}>
                <i className={`ti ${step.icon} text-base`}></i>
              </div>
              <div className={`text-xs font-medium mb-1 text-${step.color}-600`}>
                {step.num}. {step.label}
              </div>
