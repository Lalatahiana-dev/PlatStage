...
const statusConfig = {
  PENDING: { label: 'En attente', color: 'bg-yellow-50 text-yellow-600', icon: 'ti-clock' },
  CONFIRMED: { label: 'Confirmé', color: 'bg-green-50 text-green-600', icon: 'ti-circle-check' },
  CANCELLED: { label: 'Annulé', color: 'bg-red-50 text-red-500', icon: 'ti-circle-x' },
};
...
export default function StudentInterviewsPage() {
  ...
  const fetchInterviews = async () => {
    try {
      ...
      for (const app of apps) {
        try {
          const res = await api.get(`/interviews/application/${app.id_application}`);
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-800 mb-1">Mes entretiens</h1>
        <p className="text-sm text-gray-500">Suivez vos entretiens planifiés.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-8 text-center">
          ...
          <p className="text-sm text-gray-400">Aucun entretien planifié pour le moment.</p>
        </div>
      ) : (
